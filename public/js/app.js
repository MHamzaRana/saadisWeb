...
__webpack_require__(/*! ./mycustom.js */ "./resources/js/mycustom.js");
...
function addToCart(id) {
  ...
  console.log(items);
  if (!items) items = [];
  if (items.indexOf(id) < 0) items.push(id);else alert("Item is already added!");
  localStorage.setItem("items", JSON.stringify(items));
  $(".cart_remove[data-id=".concat(id, "]")).show();
  $(".cart_add[data-id=".concat(id, "]")).hide();
  checkCart();
}
function removeFromCart(id) {
  ...
  console.log(items);
  if (!items) items = [];
  if (items.indexOf(id) >= 0) items.splice(items.indexOf(id), 1);else alert("Item is already removed!");
  localStorage.setItem("items", JSON.stringify(items));
  $(".cart_add[data-id=".concat(id, "]")).show();
  $(".cart_remove[data-id=".concat(id, "]")).hide();
  checkCart();
}
